...
RC IndexMeta::from_json(const TableMeta &table, const Json::Value &json_value, IndexMeta &index)
{
  ...
  const Json::Value &field_names = json_value[FIELD_FIELD_NAMES];
  if (!field_names.isArray()) {
    ...
    if (field_value.isString()) {
      ...
      LOG_ERROR("Field names of index [%s] is not array or string.", name_value.asCString());
      return RC::INTERNAL;
    }
  } else {
    for (int i = 0; i < (int)field_names.size(); ++i) {
      const Json::Value &field_value = field_names[i];
      if (!field_value.isString()) {
        LOG_ERROR("Field name at index %d is not string.", i);
        return RC::INTERNAL;
      }
      const FieldMeta *field = table.field(field_value.asCString());
      if (nullptr == field) {
